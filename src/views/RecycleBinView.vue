...
async function onPurge(item: RecycleItem) {
  const ok = await confirmAction({
    title: "彻底删除",
    message: `彻底删除「${item.label}」？快照与图片文件将一并清除，不可恢复。`,
    tone: "danger",
    confirmText: "彻底删除",
  });
  if (!ok) return;
...
async function onClearAll() {
  const ok = await confirmAction({
    title: "清空回收站",
    message: "清空回收站？所有快照与图片文件将彻底删除，不可恢复。",
    tone: "danger",
    confirmText: "清空",
  });
  if (!ok) return;
...
<template>
  ...
  <div class="scroll-thin flex-1 overflow-y-auto p-8">
    <div class="max-w-[860px] space-y-5">
      <p v-if="error" class="rounded-md bg-danger-soft p-3 text-caption text-danger">
        {{ error }}
      </p>
      ...
      >
        <!-- 类型图标：颜色走文字 token（与 primary 同源） -->
        <div
          class="flex h-11 w-11 shrink-0 items-center justify-center rounded-[11px] bg-parchment text-primary"
          :title="item.entity_type === 'class' ? '班级' : '学生'"
        >
          <!-- 班级：讲台板书 -->
          <AppIcon v-if="item.entity_type === 'class'" name="podium" :size="22" />
          <!-- 学生：头像 -->
